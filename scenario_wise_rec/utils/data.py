...
class TorchDataset(Dataset):
    ...
    def __getitem__(self, index):
        return {k: v[index] for k, v in self.x.items()}, self.y1[index], self.y2[index]
...
class DataGenerator(object):
    def __init__(self, x, y1, y2):
        super().__init__()
        self.dataset = TorchDataset(x, y1, y2)
        self.length = len(self.dataset)

    def generate_dataloader(self, split_ratio=None, batch_size=16, num_workers=8):
